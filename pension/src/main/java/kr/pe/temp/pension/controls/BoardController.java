...
public class BoardController {
	...
	public String getform(@RequestParam Map<String, Object> param,
		...
		int totalCount = boardDao.selectTotalCount(String.valueOf(param
				.get("type")));
		int currentPage = 1; //1페이지 부터 보여주도록 현재 페이지 값 1로 설정 
		if (param.get("currentPage") != null)
			currentPage = Integer.valueOf(param.get("currentPage").toString()); //현재 페이지가 null값이 아니면 현재 페이지를 저장
		int totalPage = pagingUtils.getTotalPage(totalCount); //전체 페이지 수
		int beginIndex = pagingUtils.getBeginIndex(currentPage); //해당 페이지의 시작 인덱스
		int endIndex = pagingUtils.getEndIndex(); //페이지에 나타낼 글 수 
		int current = (currentPage - 1) / 10; //페이지를 몇번 부터 시작할지
		int total = totalPage / 10; //페이지 갯수
	...
	@RequestMapping(value = "/TcoDel", method = RequestMethod.POST)
	public String TcoDel(Board board, @RequestParam Map<String, Object> param,
			Map<String, Object> model) throws Exception {
		boardDao.deleteComment(board);
		int currentPage = Integer.valueOf((String) param.get("currentPage"));
		if (param.get("currentPage") != null)
	...
	@RequestMapping(value = "/nextNum", method = RequestMethod.GET)
	public String nextNum(Board board, @RequestParam Map<String, Object> param,
			Map<String, Object> model) throws Exception {

		board.setBsection(String.valueOf(param.get("type")));
		System.out.println(param.get("next"));
		...
		model.put("currentPage", currentPage);
		model.put("type", param.get("type"));
		if (param.get("next").equals("next")) {
			model.put("num", boardDao.nextNum(board));
		} else {
			model.put("num", boardDao.backNum(board));
		}
		return "redirect:read.temp";
	}
}
