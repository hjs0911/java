...
<script src="../../script/reservation_guide.js"></script>
...
<body data-spy="scroll" data-target=".navbar" data-offset="50">
	...
	<div class="container-fluid">
		<br>
		<h1 align="center">
			<b id="titleReservation">RESERVATION</b>
		</h1>
			...
			<div>
				<div class="collapse navbar-collapse" id="myGuideNavbar">
					<ul class="nav navbar-nav nav-pills">
						...
						<li><a href="../realtime/status_reservation.temp"
					...
					<table class="table table-hover table-responsive" style="text-align: center;">
						...
						<tbody>
							<c:forEach var="roomInfo" items="${roomInfos }">
								<tr class="mOver" id="${roomInfo.roomCode }">
									<td>${roomInfo.roomName}</td>
									<td><c:if test="${roomInfo.coupleRoom}">커플룸 </c:if> <c:if
											test="${roomInfo.doubleLayer}">복층형</c:if> <c:if
											test="${roomInfo.spa}">스파 </c:if></td>
									<td>${roomInfo.stdNum}</td>
									<td>${roomInfo.maxNum}</td>
									<td>${roomInfo.lowSeasonWeekdayPrice}</td>
									<td>${roomInfo.lowSeasonFridayPrice}</td>
									<td>${roomInfo.lowSeasonWeekendPrice}</td>
									<td>${roomInfo.middleSeasonWeekdayPrice}</td>
									<td>${roomInfo.middleSeasonWeekendPrice}</td>
									<td>${roomInfo.highSeasonWeekdayPrice}</td>
									<td>${roomInfo.highSeasonWeekendPrice}</td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
